...
| Effort display (`effort.level` next to model, default on) | `core/config.py:show_effort` (dataclass + parse + `to_dict` + `EXAMPLE_CONFIG`), `cli/statusline.py` `effort_level` extract (isinstance-guarded `effort_data.get("level")`) + `model_suffix`; conf comment synced in `data/statusline.conf.default` + `examples/statusline.conf` (kept byte-identical by `test_config_colors.py`) | `read_config()` `show_effort` (defaults + parse), `effort_level` extract + `model_suffix`, conf comment block |
