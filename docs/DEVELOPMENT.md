...
If the version reported by `pip show context-stats` is older than `version` in `pyproject.toml` (for example a venv still holding **1.23.0** while `pyproject.toml` says **1.24.1**), the editable-install metadata is stale. Re-running the editable install clears it:
