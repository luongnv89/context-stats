...
- **Standalone render pipeline decomposed (Task 5.4)** — `scripts/statusline.py`'s ~365-line `_render` is now a ≤100-line orchestrator over extracted phase helpers (payload extraction, palette/config resolution, git+PR segments, context computation, state read/persist, final assembly), mirroring the package's ColorManager/StateFile/calculate-context structure. Config color overrides no longer mutate the module-level GREEN/YELLOW/RED globals: they resolve into an explicit immutable `_Palette` passed through the coloring helpers, so renders are stateless and safe in-process. Magic numbers moved to named shared constants in `_shared` — `AUTOCOMPACT_RATIO` (0.225), the thinking-budget display tiers, and the zone RGB ANSI codes — consumed by both implementations (`formatters/tokens.py` default, `zone_ansi_code`, the package traffic-light map). Rendered output is byte-identical, pinned by new golden fixtures; zero `global` statements remain (#145)
